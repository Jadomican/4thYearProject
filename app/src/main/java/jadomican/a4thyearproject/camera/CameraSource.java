...
public class CameraSource {
    ...
    // Must assign a name to surface texture
        ...
        synchronized (mCameraLock) {
            if (mCamera != null) {
                try {
                    ...
                    Toast.makeText(mContext, "Permissions not granted for Camera!", Toast.LENGTH_SHORT).show();
                }
            }
     ...
     * The surface holder utilises a SurfaceView, which is similar to a regular View object, but can
     * be rendered on a separate GUI thread, allowing for faster processing. Used in this project
     * for the camera preview which must be updated rapidly as a user scans text.
     *
